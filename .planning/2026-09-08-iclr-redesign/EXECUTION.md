...
## 0.9 两条生成链在同一秒同时慢下来,和一个外来 CPU 作业对齐(2026-09-10 08:5x)

§0.8 刚写完就有了第一个反例方向的数据。`step-00048` 的 `naive.generate`
花了 **1.76 h**,而 step-16/24/32/40 是 1.28 / 1.22 / 1.17 / 1.18 h——
那四个稳在 ±5% 以内,这一个直接 +49%。**不是漂移,是断口。**

**不是卡住,是每张图都慢。** 按 256 张 PNG 的 mtime 差分:

| | step-00040 | step-00048 |
|---|---|---|
| 中位 | 15.33 s/img | 20.72 s/img |
| 最大间隔 | 22.8 s | 43.0 s |
| >30 s 的间隔 | **0** | **37** |

**断口落在 07:26:15,两臂、两张卡同时。** 在**同一个 stage 内部**劈分——
同一个进程、同一个模型、同一批 256 张图,只有机器上的别的租户变了:

| arm(卡)| step-40 全程 | step-48 07:26 前 | step-48 07:26 后 |
|---|---|---|---|
| naive(cuda:0)| 15.33 s/img (n=255) | 17.32 (n=97) | **27.53 (n=158)** |
| rfo_gold(cuda:1)| 20.44 s/img (n=255) | 24.23 (n=70) | **34.32 (n=148)** |

07:26:15 起的进程,只有 `027_diffsionretrieval` 的
`scripts/iclr/radgraph_second_labeller.py --cuda -1`(pid 11408,系统 Python312,
**CPU-only**,实测稳定 2.75/20 核,父进程是 Claude Code 的 bash)及其 bash 外壳。
07:00–07:40 之间别的新进程全是 node/cmd 一类的 Claude 基础设施。

**瓶颈不在卡上,也不在盘上**,这是排除项而不是解释:

- 两张卡此刻 GPU 利用率 18–37%,降频原因 `0x1`(**GpuIdle**),温度 54–59 °C。
  卡在等活干,不是算力不够,也不是热降频。
- 所有物理盘 `% Idle Time` ≥ 99.6%,平均队列长度 0.00,H: 在内。**不是 I/O**。
- 全机忙 5.81/20 核。

**不声称因果。** 2.75/20 核造成 +59% 需要一条我没测到的临界路径,
也可能有共同的第三因。**时间相关性是实测的,机理不是**——
这一条和 §0.4 那次「Windows 驱动往主机内存溢出」的教训是同一条:
测量站得住,解释不许超过测量。

**先前算错并已丢弃的一个猜测**:我自己在 06:12–08:40 之间跑了变异测试
(13 次 pytest)、若干分析脚本和 git 推送。量级上不成立——那些合计只有几分钟 CPU,
解释不了 35 分钟。写在这里是因为它是第一个想到的解释,而它是错的。

**还没解释的残差**:step-48 在 07:26 **之前**就已经比 step-40 慢 13%(naive)/
19%(rfo_gold)。那部分与这个外来作业无关,和 2026-09-09 记的
「detect 在干净对照下仍慢 15–47%」大概率是同一个家族。**仍然没有原因。**

**代价**:按 07:26 之后的速率,慢链 rfo_gold 每块多约 59 分钟。
剩 5.5 块 ≈ **关键路径 +5.4 h**,足以吃掉 §0.8 的整个悲观端(+0.9 h)。

**做了什么,没做什么。** 没有碰 pid 11408(它不属于本项目)。
**也没有调本项目进程的优先级**——stage 墙钟本身是证据(§0.3 排班基准、§0.5–0.8 的余量),
运行途中插一个没登记的干预,会在时间记录里留下一个我自己造的断口。
做的是:给 027 session 发了一条实测通报,写明不要求他们停、我不会碰他们的进程,
并给了两个对他们几乎无成本的选项(`Below Normal` 优先级,或者告诉我预计什么时候结束)。
那个 session 07:27:05 起就闲置了,消息会排队。

**可证伪的预测**:若那个作业结束或降优先级,per-image 应回到约 15.3(naive)/
20.4(rfo_gold) s/img。**回不去,就说明这次相关性是假的**,§0.9 的归因作废,
残差整体退回「未解释」。下一块 `step-00056` 的生成链是第一个检验点。
