...
rfo_gold step-00000 整条链          +4.15 h(同上,两臂本应并行)
```

两臂各占一张卡,但**一次双臂评测的墙钟由慢的那条决定,约 4.2 h,
而不是单臂的 1.9 h**。§2b 的并行化把两条链拆开了,可它们在检测阶段
...
```
11 × 1.77 (train) + 12 × 4.2 (evaluate)  ≈  70 h
```
...
70 h 落在两者之间。**supervisor 不会停(配置是 96),但 E3 注册的失败条件 4
