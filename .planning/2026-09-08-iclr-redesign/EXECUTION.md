...
| seed 数 | **已改判:补到 3 seed**(2026-09-09,算力与时间充足;偏离 8 取代 6.1)。取值定死 20260906 / 20260907 / 20260909 | 等 A800 |
| `training.seed` 旋钮 | **必须先加**(偏离 8.5):v4 只有顶层 `seed`,它同时管 split、LoRA init、评测 latent;直接改会让三个 seed 落在不同划分和不同 latent 上,配对全废。additive 标量,缺省等于顶层 seed,改在 arm B worktree | 代码,不占卡 |
