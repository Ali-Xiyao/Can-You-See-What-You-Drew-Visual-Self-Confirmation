...
## 6. 发表判据:在看到 E3 任何数字之前先写下来(2026-09-09)

你让我最后判断这个能不能发 ICLR 2028。**判据现在写,不等结果**——
否则我会对着出来的数字挑一个能自圆其说的档位,那和预注册整篇文件反对的
是同一件事。主运行现在只出了 12 个检查点里的 2 个,而我一个 outcome
数值都没看过(§4 的规矩)。

### 不依赖 E3 的、已经在账上的

- **E4 推理期复制,注册的三个模型 3/3**(`review-packets/cross-model-20260908/`)。
  注册集合是 `{showo2_1p5b, showo2_7b, showo_v1}`,Janus-Pro-1B 单独一行、
  不计入判定(偏离 2)。两条要在正文写清的口径:showo_v1 的效应小一个量级;
  showo2_1p5b 与 janus_pro_1b 的弃权率显著不同,两个 n 都要报。
- **上下文消融本身**:0.654 [0.617,0.689] n=667 对 0.336 [0.301,0.372] n=682。
- **离线重放的选择增益**,以及它在两个 corpus 上差 5 倍这件事。

这些加起来已经是一篇**推理期测量**的论文。E3 决定的是它能不能再加上
「训练中会恶化」和「BSV 能挡住」。

### 判据表(端点 1 与端点 2 的四种组合)

| 端点 1:B−A 外部正确率 | 端点 2:盲测判别 gap 斜率 | 能主张的最强版本 | 我会给的判断 |
|---|---|---|---|
| ≥ +0.042 且 CI 不含 0 | A 显著为负,B 显著高于 A | 测量 + 机制 + 零成本修复,三段齐全 | **够 ICLR 主会**,且是强投稿 |
| 未测到 | A 显著为负,B 显著高于 A | 机制成立,修复只到「选择信号质量」这一层 | **够主会,但要靠 E4 撑泛化**;标题不能写 improves generation |
| ≥ +0.042 且 CI 不含 0 | 两臂都塌 | 有效果,但 §2 的机制解释被自己的实验证伪 | **边缘**。story 必须重写成「BSV 有效,原因未明」,这在主会很难过,更像 workshop |
| 未测到 | 两臂都塌 | 只剩推理期的测量与消融 | **不够主会**。E4 + 消融那部分仍然是好 workshop / findings 文章,不要硬投 |

端点 3(剂量-反应)不参与档位判定:斜率不显著为正就按预注册写成
「未解释的调节效应」,它只影响 discussion 的强度。

失败条件任一触发(墙钟、`blind()` 守卫抛错、同阶段连续两次不可恢复失败)
→ 按预注册降级,**不靠加算力、加 seed、换 backbone 抢救**。

### 不管结果怎样都成立的三个减分项

1. **单 seed。** 端点 2 的「斜率显著」是 over prompt 的 bootstrap,不是
   over seed;它支持的是「在这 64 个 prompt 上」,不是「在这个训练过程上」。
   这是最大的单点风险,而且**预算里没有第二个 seed**(预注册 §4.2 已承认)。
   审稿人会打,而且打得对。诚实写进 limitations 是唯一的处理方式。
2. **合成场景,n=64。** 颜色 × 计数 × 物体的组合题,外部效度有限。
3. **BSV 不是新模块**,是把一段文本从上下文里删掉。贡献必须主张成
   「测量 + 机制 + 后果」;任何把它包装成方法创新的写法都会被正确地打回。

还要加一条本次新出的:

4. **arm A 在两个运行里不是同一个东西。** 主运行的 A 与 rfo_gold 配对,
   实际只在约 75% 的 prompt 上训练;arm B 里的 A 与 blind_self 配对,
   交集不同。「独立复制」这个说法要讲清楚复制的是什么(同一 split、同一
   config、不同配对),不能让读者以为是同一条曲线跑了两遍。
   还有 §5 那条:两个运行的敏感性分析不同源。

### 我事后不会做的事(现在写下来,好让将来能被抓住)

- 不动 +0.042 这个阈值,不改成「趋势」「接近显著」。
- 不把 Janus 挪进 E4 的分母去凑「三个里两个」。
- 不把主运行的事后子集说成预注册冻结(§5 的陷阱)。
- 不因为曲线好看就把单 seed 的轨迹写成显著性结论。
- 不在端点 2 塌了之后换一个 gap 的定义再算一遍。
