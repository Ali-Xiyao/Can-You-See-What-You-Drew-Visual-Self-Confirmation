...
「Tier B 的题太清楚所以不服从」这个解释**是错的**：counting 的诊断题在 image-only 下也有 0.825，一点不含糊，prompted 下却掉到 0.249。像素清楚而服从极强的格子存在。留下的候选解释是错法不同（counting 要断言的是一个不在图里的物体，改色问的是在图里的物体）、可归因性（一只毫不含糊的绿杯子不是模型合理会犯的错，少画一个梨是）、以及编辑痕迹。**能分辨它们的编辑正是这一轮做不了的那个——删除物体**，它要 inpainting 权重。
