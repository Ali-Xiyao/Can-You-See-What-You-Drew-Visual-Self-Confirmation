...
**Version 3.3 · 2026-09-01 · 目标会议 ICLR 2028**
...
| Tier B 像素反事实 | 300 对（翻转 + 改色） | 因果证据；补 spatial 与 binding 的诊断供给 |
...
可用底料：两个半边合计 912 张图里，外部验证为正确且不待裁定的有 **209** 张，其中 207 张有两个以上可分辨 x 位置的物体（翻转可用），色词落在词表内且带框的物体实例 591 个（其中有色相的 454 个）。目标 300 对，供给不是瓶颈。
