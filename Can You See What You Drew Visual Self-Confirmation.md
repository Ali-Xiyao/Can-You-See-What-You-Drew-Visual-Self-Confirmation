...
**Version 3.1 · 2026-09-01 · 目标会议 ICLR 2028**
...
| **②  机理** | 分叉主要由意图泄漏驱动，共享感知盲点为次因 | 静态配对对照已测得泄漏本身（§3.4，0.651 → 0.201）；上下文消融对 D_g / D\* 的位移量分解；per-block 梯度余弦定位 |
...
117 条 spec × K=4 = 468 张图，1783 道 2AFC 配对试次。同一批图、同一批题、同一个模型，唯一差别是问题前面有没有那句「你是照这个描述画的：……」：

| | image-only | prompted | 配对 McNemar |
|---|---:|---:|---|
| 图与 spec 一致（1244 道） | 0.982 | **0.993** | 3 → 17 反向，χ²=8.4，p=0.0037 |
| 图与 spec 冲突（307 道） | 0.651 | **0.201** | **140 → 2，χ²=132，p=1.4×10⁻³⁰** |

**两行方向相反，这是全部的分量所在。** 若那句描述只是干扰上下文、只是加长了 prompt，两行都该下降。实际上图与描述一致时它略有帮助（+0.011，显著），只有在像素与描述冲突时才崩塌：加上描述后 140 道从对变错，只有 2 道从错变对。自我确认率（诊断性试次中按被要求的场景而非画出的场景作答的比例，随机 = 0.5）**0.349 → 0.799**。

counting 族最干净：0.815 → 0.109（n=135）。不给描述时模型数得清自己画的三个梨；给了描述之后，尽管问题明确要求「回答图里实际有什么」，它有 89% 的时候答「两个」——那是它被要求画的数量。**这不是看不见，是看见了不采信。** absence 0.510 → 0.262（n=145）；binding（n=16）与 existence（n=11）方向一致但 n 太小，不单独下结论。

程序性检查：prompted 条件下弃答率 0.000，选 A 比例 0.499（gold 中 A 占 0.496），无位置偏置；排除 28 张待人工裁定的图后两个诊断率变为 0.6445 / 0.2020，结论不变。
...
**边界**：只有 `2+1` 半边、单一模型、单一 seed 组，描述性结果，不是训练结论。`1+1+1` 半边同协议复跑中。
