...
`showlab/show-o2-1.5B-HQ`（rev `d3a220ec`）为主 backbone。Show-o2-7B 已审计：Oracle@4 提升到 0.92，但首候选整体覆盖率降到 0.767，固定种子覆盖摆动恶化到 16pt（HQ 为 10pt）。**放大规模没有修复生成域可测量性**，7B 与 Show-o v1 一并作为冻结负对照。
...
**规模旋钮由实测确定，不是选的。** 48 条 prompt × K=4 候选过 Show-o2-1.5B-HQ，由 Qwen3-VL-8B 按多重集规则评分：
...
**Tier B 构造**：选外部验证为正确的生成图 → 对一个原子属性做最小可控编辑（删除对象、改颜色、左右互换）→ 保持原 prompt 不变 → 用 2AFC 询问实际事实。真值完全由构造过程给出。
...
| 族 | 诊断性试次的构造 | 干扰项来源 | 实测诊断率（468 张图） |
|---|---|---|---:|
| counting | 画了三个，要求两个 | 请求的数量 | 0.349 |
| absence | 要求蜡烛，一根没画 | 未画出的请求物体 | 0.340 |
| existence | 多画了瓶子、漏画了蜡烛 | 未画出的请求物体 | 0.173 |
| binding | 要求绿梨，画成红梨 | 请求的颜色 | 0.034 |
| spatial | 要求杯在苹果左，画在右 | spec 关系 | 0（本语料不带关系） |

整体诊断性试次占比 **0.223**（1860 题中 414 道）。三点必须在论文里写明：
...
- **spatial 在本语料上恒为 0，且是有意的。** 让它变成诊断题必须在 spec 里带关系，而带关系等于多给生成器一个必须做对的约束，直接压 p——p 是这条路线上最稀缺的资源。不带关系的 prompt 让 spatial 题「白拿」，代价是它只测「能否读出自己画的布局」，不测「是否服从 prompt」。
