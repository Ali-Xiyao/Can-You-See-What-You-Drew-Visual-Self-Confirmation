...
| **②  机理** | 分叉主要由意图泄漏驱动，共享感知盲点为次因 | 静态配对对照已测得泄漏本身（§3.4，0.661 → 0.336，n=672）；上下文消融对 D_g / D\* 的位移量分解；per-block 梯度余弦定位 |
...
228 条 spec × K=4 = 912 张图，3672 道 2AFC 配对试次（`2+1` 与 `1+1+1` 两个半边独立跑完同一条流水线）。同一批图、同一批题、同一个模型，唯一差别是问题前面有没有那句「你是照这个描述画的：……」：

| | image-only | prompted | 配对 McNemar |
|---|---:|---:|---|
| 图与 spec 一致（2355 道） | 0.978 | **0.990** | 11 → 39 反向，χ²=14.6，p=1.3×10⁻⁴ |
| 图与 spec 冲突（672 道） | 0.661 | **0.336** | **227 → 9，χ²=200，p=2.6×10⁻⁴⁵** |

**两行方向相反，这是全部的分量所在。** 若那句描述只是干扰上下文、只是加长了 prompt，两行都该下降。实际上图与描述一致时它略有帮助，只有在像素与描述冲突时才崩塌：加上描述后 227 道从对变错，只有 9 道从错变对。自我确认率（诊断性试次中按被要求的场景而非画出的场景作答的比例，随机 = 0.5）**0.339 → 0.664**。

**两个半边各自成立，反向那一行也各自成立**（一致行 +0.011 与 +0.013，冲突行 −0.450 与 −0.219）。分族（合并）：counting 0.825 → 0.249（n=189）、binding 0.792 → 0.500（72）、existence 0.571 → 0.333（42）、absence 0.561 → 0.350（369）；四族全部显著同向。
...
程序性检查：prompted 条件下弃答 0 道，image-only 下 49 道——**那句描述不只改变答案，它还让模型每次都给出答案**；无位置偏置（选 A 0.499 / 0.525，gold A 0.496 / 0.510）；排除待人工裁定的图后结论不变。
...
**边界**：单一模型、单一 seed 组，描述性结果，不是训练结论。70 张图（占 7.7%）仍待人工裁定，其金标准可能变动；排除它们不改变任何一行的方向或显著性。配对分析见 `scripts/v4_compare_conditions.py`——`v4_analyze.py` 一次只看一个条件，而主张在两个文件的逐试次差上。
...
自然池上诊断性试次的实际供给决定了 Tier B 要补哪几族。912 张图、3721 道题里，spec 条目层面的结果是：**颜色画对 1495、画错 88、整个物体没画出来 549、同名画成两色 136**。颜色几乎免费而物体常常缺席，所以 absence（369 道诊断）与 counting（200 道）够用，binding（72）与 existence（47）稀薄，spatial 为 0。
...
|---|---|---|---:|---:|
| absence | 要求蜡烛，一根没画 | 未画出的请求物体 | 0.423 | 369 |
| counting | 画了三个，要求两个 | 请求的数量 | 0.230 | 200 |
| binding | 要求绿梨，画成红梨 | 请求的颜色 | 0.090 | 72 |
| existence | 多画了瓶子、漏画了蜡烛 | 未画出的请求物体 | 0.054 | 47 |
| spatial | 要求杯在苹果左，画在右 | spec 关系 | 0 | 0（本语料不带关系） |

整体诊断性试次占比 **0.185**（3721 题中 688 道）。三点必须在论文里写明：
