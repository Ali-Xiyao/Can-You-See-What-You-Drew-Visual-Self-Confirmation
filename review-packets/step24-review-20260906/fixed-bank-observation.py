...
def build(run_dir):
    inputs = Inputs()
    prior_audits = []
    audit_paths = [run_dir / "runtime-check" / name for name in (
        "base-gradient-integrity.json", "naive-step00008-gradient-integrity.json",
        "rfo-gold-step00008-gradient-integrity.json", "naive-step00016-gradient-integrity.json",
        "rfo-gold-step00016-gradient-integrity.json",
    )] + [Path(__file__).with_name("gradient-audit.json")]
    for audit_path in audit_paths:
        audit = inputs.json(audit_path)
        if audit.get("status") != "pass" or audit.get("must_pause", False) or audit.get("must_pause_for_integrity", False):
            raise ValueError(f"Prior checkpoint audit is not a pass: {audit_path}")
        prior_audits.append({"path": str(audit_path.resolve()),
                            "sha256": inputs.hashes[str(audit_path.resolve())],
                            "status": "pass"})
    for relative in (
    ...
    for arm, step in checkpoint_specs:
        ...
        for image_key, candidate in images.items():
            ...
            if row is not None and row_status is None:
                ...
                actual = indexed(observation.get("answers") or [], lambda a: a["question_id"],
                                 "saved atomic answer")
        ...
        checkpoints.append({
            ...
            "coverage": {"expected_images": 80, "observed_images": len(observed),
                         ...
                         "observed_atomic_answers": sum(len((x.get("observation") or {}).get("answers") or [])
                                                        for x in observed.values()
                                                        if isinstance(x.get("observation"), dict)),
    ...
    inputs.verify()
    return {
        "schema_version": "fixed-bank-observation-2",
        ...
        "prior_checkpoint_identity_audits": prior_audits,
        "checkpoint_identity_scope": "Checkpoint weight and gradient integrity are inherited from the cited completed audits; this answer-only diagnostic does not repeat weight or Gram audits.",
