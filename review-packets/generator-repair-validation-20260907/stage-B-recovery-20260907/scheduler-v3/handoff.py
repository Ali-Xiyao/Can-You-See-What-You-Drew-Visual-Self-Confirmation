...
import sys, os, time, json, subprocess, hashlib, ctypes, ntpath
import psutil
...
def system_directory():
    ...
    assert 0 < length < len(buffer), 'Cannot determine Windows system directory'
    return buffer.value


def console_path(path):
    # Windows reports the command executable with an NT namespace prefix;
    # psutil.exe() gives its ordinary absolute path.
    for prefix in ('\\??\\', '\\\\?\\'):
...
def console_helper(p, old_identity, system_dir=None):
    """Exclude only the original dispatcher's observed Windows console helper."""
    expected = console_path(ntpath.join(system_dir or system_directory(), 'conhost.exe'))
    item = identity(p)
    cmd = item['cmdline']
    if (console_path(p.exe()) != expected or len(cmd) != 2
            or console_path(cmd[0]) != expected or cmd[1] != '0x4'
            or p.ppid() != old_identity['pid']
            or not 0 <= item['create_time'] - old_identity['create_time'] <= 5):
        return None
    return {**item, 'exe': p.exe(), 'parent_pid': p.ppid(),
            'reason': 'system_console_helper_created_with_old_controller',
            'adopted': False, 'terminated_by_handoff': False}
...
def main():
    ...
    try:
        ...
        jobs = []; ignored_console_helpers = []
        for child in old.children(recursive=False):
            if child.is_running():
                helper = console_helper(child, before)
                if helper is not None:
                    ignored_console_helpers.append(helper)
                else:
        ...
        save_new(S / 'handoff.json', {'schema_version': 1, 'created_utc': now(),
            ...
            'ignored_console_helpers': ignored_console_helpers,
