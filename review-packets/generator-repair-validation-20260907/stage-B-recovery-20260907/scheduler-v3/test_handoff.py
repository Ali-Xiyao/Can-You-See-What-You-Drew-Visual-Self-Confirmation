...
class ConsoleProcess:
    ...
    def __init__(self, exe=r'C:\Windows\System32\conhost.exe',
                 cmd=None, created=100.1, parent=7700):
        self.executable = exe
        self.command = cmd or [r'\??\C:\Windows\system32\conhost.exe', '0x4']
        self.created, self.parent = created, parent
...
OLD_ID = {'pid': 7700, 'create_time': 100.0, 'cmdline': ['python', 'launch_recovery.py']}


def test_exact_system_console_helper_is_excluded_and_identity_retained():
    result = handoff.console_helper(ConsoleProcess(), OLD_ID, r'C:\Windows\System32')
    assert result['pid'] == 14592 and result['parent_pid'] == 7700
    assert result['adopted'] is False and result['terminated_by_handoff'] is False
    assert result['cmdline'] == [r'\??\C:\Windows\system32\conhost.exe', '0x4']


@pytest.mark.parametrize('process', [
    ConsoleProcess(exe=r'C:\temp\conhost.exe'),
    ConsoleProcess(cmd=[r'C:\temp\conhost.exe', '0x4']),
    ConsoleProcess(cmd=[r'C:\Windows\System32\conhost.exe', 'unexpected']),
    ConsoleProcess(parent=123),
    ConsoleProcess(created=110.0),
])
def test_impostor_or_other_console_is_not_ignored(process):
    assert handoff.console_helper(process, OLD_ID, r'C:\Windows\System32') is None
    with pytest.raises(AssertionError, match='Unexpected direct child'):
        handoff.classify(process)
