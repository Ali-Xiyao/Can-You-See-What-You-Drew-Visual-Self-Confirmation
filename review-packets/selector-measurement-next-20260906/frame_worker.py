"""Registered extra condition: preserve framing and remove description content only."""
import argparse
...
def main():
    ...
    for path,digest in read(SIDE/'frame-source-freeze.json').items():
        assert sha(path)==digest,path
    plan={**plan, 'conditions':['prompt_blank']}
    assert_main_paused()
    ...
    for task in [t for t in plan['tasks'] if t['device']==args.device]:
        ...
        folder=SIDE/'matched-frame'/task['id']
            ...
            for pool in bank['pools']:
                ...
                for c in pool['candidates']:
                    ...
                    for condition in order:
                        ...
                        seed_training(seed)
                        questions=tuple(replace(q,text=PROMPTED_PREAMBLE.format(prompt='',question=q.text)) for q in raw_questions) if condition=='prompt_blank' else raw_questions
                        ...
                        completed+=1
                    print(f"{task['id']} images {completed}/{plan['n_images']}",flush=True)
        assert completed==plan['n_images']
